...
describe 'zabbix_extras::agent::beegfs' do
  ...
  it "should create UserParameter for beegfs" do
    ...
    content.split("\n").reject { |c| c =~ /(^#|^$)/ }.should == [
      ...
      'UserParameter=beegfs.storage.reachable[*],beegfs-ctl --listnodes --reachable --nodetype=storage | grep -A1 \'\[ID: $1\]\' | grep -c "Reachable: <yes>"',
      'UserParameter=beegfs.client.reachable[*],beegfs-ctl --listnodes --reachable --nodetype=client | grep -A1 \'\[ID: $1\]\' | grep -c "Reachable: <yes>"',
